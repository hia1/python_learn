...
class Crawler(object):
    ...
    def run(self):
        ...
        for index, url in enumerate(self.parse_menu(self.request(self.start_url))):
            ...
            htmls.append(f_name)
        try:
            pdfkit.from_file(htmls, self.name+ ".pdf", configuration=config,options=options)
        except Exception as e:
            logging.error(print(e), exc_info=True)
        finally:
            for html in htmls:
                os.remove(html)
...
class LiaoxuefengPythonCrawler(Crawler):
    ...
    def parse_menu(self, response):
        ...
        # print('=======',menu_tag)
        for li in menu_tag.find_all("a",class_="x-wiki-index-item"):
            print(li)
            url = li.get("href")
...
if __name__ == '__main__':
    ...
    crawler = LiaoxuefengPythonCrawler("liaoxuefengpdf", start_url)
    crawler.run()
